...
if (document.querySelectorAll('#'+id+' tbody tr').length < 6) {  // чтобы при перелистывании месяцев не "подпрыгивала" вся страница, добавляется ряд пустых клеток. Итог: всегда 6 строк для цифр
  ...
  Calendar2("habitInfo_table_calendar", new Date().getFullYear(), new Date().getMonth());
  // переключатель минус месяц

  document.getElementById('habitInfo_leftButton').addEventListener('click',function() {
    Calendar2("habitInfo_table_calendar", document.getElementById('habitInfo_month').dataset.year, parseFloat(document.getElementById('habitInfo_month').dataset.month)-1);
  })
  // переключатель плюс месяц
  document.getElementById('habitInfo_rightButton').addEventListener('click',function() {
    Calendar2("habitInfo_table_calendar", document.getElementById('habitInfo_month').dataset.year, parseFloat(document.getElementById('habitInfo_month').dataset.month)+1);
  })
